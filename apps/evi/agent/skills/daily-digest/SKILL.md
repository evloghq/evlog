...
4. **Traffic (last 24h).** Browser pageviews: `get_web_analytics` in `count` mode, one call per project (`projectId`, `teamId`, `since`, `until`), labeled as browser traffic. Agent-facing requests (MCP transport, `.md`, negotiated Markdown): the ecosystem-usage skill, reported in its own line, never mixed with browser counts.
