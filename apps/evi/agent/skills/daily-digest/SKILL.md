...
3. **Visitors (last 24h).** `vercel__get_web_analytics` in `count` mode with `dataset: 'visits'`, one call per project (`projectId`, `teamId`, `since`, `until`), for the docs site and the other evlog projects.
