...
5. **Push the branch with `git__push`.** That tool is the only way code reaches the remote: never the GitHub file API. It refuses `main` and `master`, and only maintainer sessions have it. For a repository other than the home one, `git__checkout` it first and pass the same `repository` to `git__push`; the whole flow then runs in that checkout instead of `/workspace/repo`.
