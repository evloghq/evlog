...
Catalog errors are wire format (see [Structured Errors](/libraries/errors)), so assert through the factory rather than a string literal. A rename then fails in your test suite instead of in a host's alert or an agent's prompt:
...
If you generate an error reference from the catalog, the committed file drifts the first time someone edits a `fix` and forgets to rebuild. One test closes that gap: regenerate in memory and compare to what is on disk. [Exporting the Catalog](/libraries/exporting) has the script and the test; the assertion is `expect(committed).toBe(toMarkdown(readCatalog()))`, and it fails in the pull request that changed the sentence instead of in a consumer's session a week later.
