...
description: Use evlog for individual structured messages, with the same drains and redaction as wide events, and adapt existing logger call sites.
navigation:
...
Writing a reusable library or SDK? [Libraries](/use-cases/libraries) covers the emit-only contract a package follows when it runs inside a host application.
::
...
- [Structured Errors](/learn/structured-errors): Throw errors with `why`, `fix`, and `link`
- [Configuration](/reference/configuration): All `initLogger` options
- [Adapters](/integrate/adapters/overview): Send events to Axiom, Sentry, PostHog, and more
- [Standalone TypeScript](/integrate/frameworks/standalone): Scripts, workers, and libraries without a web framework
- [evlog vs other loggers](/reference/vs-other-loggers): Side-by-side with pino, winston, consola
