...
A catalog is just regular TypeScript that depends on `evlog` as a peer dep. Here is the minimal recipe. For the logging side of shipping evlog from a package, [Libraries](/use-cases/libraries) covers emitting without owning the configuration and attributing events to your package.
