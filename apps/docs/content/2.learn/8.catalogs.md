...
Docs: https://www.evlog.dev/learn/catalogs

::

::tip
If you haven't yet, start with [Structured Errors → Error Catalogs](/learn/structured-errors#error-catalogs) and [Audit → defineAuditCatalog](/use-cases/audit/recording#defineauditcatalog) for the basics. This page assumes you've used the primitives at least once.
::
...
A catalog is just regular TypeScript that depends on `evlog` as a peer dep. Here is the minimal recipe. For the logging side of shipping evlog from a package, [Libraries](/libraries/emitting) covers emitting without owning the configuration, attributing events to your package, and what error codes owe your consumers.
...
- [Structured Errors](/learn/structured-errors): The full `createError` API and `parseError` reference.
- [Audit → Recording](/use-cases/audit/recording): All audit-emission APIs (`log.audit`, `withAudit`, etc.).
- [Frameworks](/integrate/frameworks/overview): Auto-managed per-request loggers and HTTP error serialization.
