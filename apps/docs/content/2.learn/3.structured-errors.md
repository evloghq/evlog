...
When the reader of `fix` is a model calling your package as a tool, the bar is higher: it executes the sentence literally. [Errors Agents Can Act On](/libraries/agents) covers what each field has to carry for that reader.
...
**Going further.** The dedicated [Catalogs page](/learn/catalogs) covers the scaling story (single file → folder → feature → npm package), the full npm packaging recipe, composition patterns, the type-augmentation deep dive, and common pitfalls.
::

::callout{icon="i-lucide-code" color="neutral"}
See the [Next.js guide](/integrate/frameworks/nextjs) for a working implementation.
::

## Next Steps

- [Wide Events](/learn/wide-events): Accumulate context and emit comprehensive events
- [Adapters](/integrate/adapters/overview): Send errors and events to Axiom, Sentry, PostHog, and more
- [Frameworks](/integrate/frameworks/overview): Auto-managed request logging per framework
- [Quick Start](/start/quick-start): See all evlog APIs in action
