...
## Every field the middleware records
...
| `ai.prompt` | First call's `params.prompt` | Formatted text of the prompt sent to the first model call (only when the `prompt` option is enabled) |
| `ai.output` | Content / stream chunks | Text generated by the last model call (only when the `output` option is enabled) |
