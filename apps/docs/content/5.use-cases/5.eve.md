...
export default defineEvlogHook({
  enrichTurn: (ctx) => ({
    tier: ctx.session.auth.current?.principalId === 'github:1' ? 'pro' : 'free',
  }),
})
```

Returned fields merge onto the turn event, over the built-in `eve`, `agent` and `channel` fields, and stay turn-scoped: the next turn of the session does not inherit them. A throwing `enrichTurn` is logged and skipped, never fails the turn.
