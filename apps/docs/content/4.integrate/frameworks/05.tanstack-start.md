...
TanStack Start uses [Nitro v3](/integrate/frameworks/nitro) as its server layer, so evlog integrates via the `evlog/nitro/v3` module. The same plugin-based hooks system applies.

::callout{icon="i-lucide-info" color="info"}
**TanStack Router vs TanStack Start**: TanStack Router is a client-side router and doesn't need server-side logging. This page covers **TanStack Start**, the full-stack framework. If you're using TanStack Router in SPA mode, see [Client Logging](/use-cases/client-logging) instead.
::

::prompt
---
description: Set up evlog in my TanStack Start app
icon: i-custom-tanstack
actions:
  - copy
  - cursor
...
Docs: https://www.evlog.dev/integrate/frameworks/tanstack-start
...
**Using Vite?** TanStack Start is Vite-based. The [`evlog/vite`](/reference/vite-plugin) plugin strips `log.debug()` from production builds and injects source locations, add it to your `vite.config.ts` alongside the TanStack Start plugin.
::
...
`log.fork()` is not available in this integration. The `evlog/nitro/v3` module attaches the logger to the request event and registers no `AsyncLocalStorage` storage of its own, so there is no scope for a child wide event to run in, even with `experimental.asyncContext` enabled (that powers Nitro's `useRequest()`, not `log.fork()`). Await background work inside the handler instead; mutating the logger after the wide event has emitted triggers a post-emit warning. See [After emit](/learn/wide-events#after-emit-sealing-and-background-work) in Wide events.
