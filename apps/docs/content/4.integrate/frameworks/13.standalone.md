...
For scripts, queue workers, cron, and CLIs, this page is the reference. On Cloudflare Workers, prefer [Cloudflare Workers](/integrate/frameworks/cloudflare-workers) (`createWorkersLogger`).
::

::callout{icon="i-lucide-package" color="neutral"}
Shipping evlog inside an npm library or SDK instead? [Libraries](/use-cases/libraries) covers what a package may emit and what it leaves to the host application.
::

::prompt
---
description: Set up evlog in my TypeScript project
icon: i-simple-icons-typescript
actions:
  - copy
  - cursor
...
Docs: https://www.evlog.dev/integrate/frameworks/standalone
...
**Using vite-node?** The [`evlog/vite` plugin](/reference/vite-plugin) replaces the `initLogger()` call with compile-time auto-initialization, strips `log.debug()` from production builds, and injects source locations.
::
...
::callout{icon="i-lucide-info" color="info"}
See the [Adapters](/integrate/adapters/overview) docs for all available drain adapters (Axiom, OTLP, PostHog, Sentry, Better Stack).
::
...
- [Node.js request logging](/use-cases/structured-logging-nodejs): apply these primitives in a complete HTTP server and trace success and failure from the response to the log.
