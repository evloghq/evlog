...
`evlog map` is not a benchmark or a flaky measure of your app at runtime. It is a **deterministic static check** of agreed logging conventions on the entry points its adapters recognize: the kind of check you can gate a pull request on.
...
- **Gates without flakiness.** Because the result cannot vary run to run, `--min-score <n>` behaves like a real test: a green gate means the recognized requirements cleared the floor you set, not that the runner got lucky. A deterministic score is the only kind you should ever block a pull request on.
...
It also reports **before any traffic exists**. Unlike an agent that has to watch a live app, `evlog map` finds missing recognized patterns the moment the code is written, which is when fixing them is cheap. Use it as a day-one convention check so you are not discovering those gaps mid-incident.
