---
title: 'Observability Score with evlog map'
...
This command inspects the project without writing `evlog.map.json`. The examples on this page were checked with CLI 0.6.3:

```bash [Terminal]
npx @evlog/cli@0.6.3 map --no-write
```
...
This output comes from the repository's Next.js example at commit `e154fa9c`. It has five detected entry points, two with failed requirements:
...
```bash [Terminal]
npx @evlog/cli@0.6.3 map app/api/hello/route.ts --no-write
```
