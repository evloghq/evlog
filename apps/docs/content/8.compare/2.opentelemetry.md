...
The function writes a completed checkout event as local JSON and exports it through OTLP. `await drain({ event })` waits for the send to finish, but a failed send does not throw: the drain logs it as `[evlog/otlp] Failed to send events:` and resolves. Call `drain.raw({ event })` instead if you own the failure handling, because `raw` rejects on a failed send. Do not also register this drain globally, which would export the event twice.
...
This example waits for the network request. For production handler integration, wrap the drain in [createDrainPipeline](/extend/drain-pipeline) for batching and retries, and wire its `flush()` into your framework's shutdown lifecycle. The adapter supports OTLP HTTP logs, not a gRPC-only endpoint. Serialization, batching, retries, and export add costs that event-construction microbenchmarks do not measure. See [Performance](/reference/performance) for those boundaries.
