...
- `compute(ctx)` reads from `ctx.headers` / `ctx.request` / `ctx.response` / `ctx.event` and returns the value to merge (or `undefined` to skip)
- Keep `compute` pure and fast: no awaitable I/O on the hot path; cache anything expensive at module scope
...
Docs: https://www.evlog.dev/extend/custom-enrichers
Built-in: https://www.evlog.dev/use-cases/enrichers
